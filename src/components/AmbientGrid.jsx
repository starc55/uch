...
function AmbientGrid({ mousePosition }) {
  ...
  useEffect(() => {
    const media = window.matchMedia(ambientMotionQuery);
    const updateMotionMode = () => setCanAnimate(media.matches);
  ...
  const animationsEnabled = canAnimate && !reducedMotion;
  ...
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      ...
      {animationsEnabled ? (
        ...
        <>
          <div className="absolute inset-x-[8%] top-[22%] h-px bg-gradient-to-r from-transparent via-accent/30 to-transparent" />
          <div className="absolute left-1/2 top-[16%] h-56 w-56 -translate-x-1/2 rounded-full bg-accent/14 blur-[72px]" />
        </>
      )}
    </div>
  );
